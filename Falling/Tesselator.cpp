 ...
 * Constructor
 */
Point::Point(Point2D & p) : pt(p), adjlist(0), marked(false)
{ }
...
Poly *Tesselator::triangleMarching(Point *begin, Point2D &end, Point **graphpt)
{
	...
	do
	{
		...
		{
			if(end.exactEquals(p1->pt)) 
			{
				(*graphpt) = p1;
				return 0; // point already in graph
			}
			else if(end.exactEquals(p2->pt))
			{
				(*graphpt) = p2;
				return 0; // point already in graph
			}

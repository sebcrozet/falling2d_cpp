...
#if 0
  int ImplicitShape::getMarginedContinuousSupportPoint(const Vector2D &d, Point2D *res) const
  {
    int opt = getContinuousSupportPoint(d, res);
    addMargin(d, res);
    return opt;
  }

  int ImplicitShape::getMarginedContinuousSupportPoint(const Vector2D &d, Point2D *res, int optimisationId) const
  {
    int opt = getContinuousSupportPoint(d, res, optimisationId);
    addMargin(d, res);
    return opt;
  }
#endif

	...
	for(int i=0; i<collisions.size();i++)
	{
		...
		if(max)
		{
			...
			for(int j=0;j<max;j++)
			{
				...
				if(/* handled by next condition: cnt->s1->isFixed() || */cnt->s1->getStackLevel() < cnt->s2->getStackLevel())
				{
					cnt->s1 = cnt->s2;
					cnt->s2 = 0;
					cnt->normal.reflect();
				}
				else if(/* handled by next condition: cnt->s2->isFixed() || */cnt->s1->getStackLevel() > cnt->s2->getStackLevel())
					 cnt->s2 = 0;
				// get tangeant

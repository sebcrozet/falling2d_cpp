...
namespace Falling
{
  ...
  void ContactBatch::addContact(const SubCollision &lastDeepestPen)
  {
    ...
    for(int i = 0; i < (int)scol.size(); i++)
    {
      ...
      if(cb->depth < -2.0 * PROXIMITY_AWARENESS ||
         (ni * ni - cb->depth * cb->depth) > 4.0 * PROXIMITY_AWARENESS *
                                                   PROXIMITY_AWARENESS)
      {
      ...
      else
      {
        ...
        /* FIXME: use more reliable value instead of PROXIMITY_AWARENESS */
        if(test * test < PROXIMITY_AWARENESS)
        {
          // replace
          cb->depth = depth;
          /*
           * FIXME: why storing the local position of contact points and
           * not simply the localposition of the real contat point (the middle) ?
           * Or only the position of the middle contact point?
           */

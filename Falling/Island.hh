...
namespace Falling
{
  ...
  class Island
  {
    ...
    private:
    int nbrCtcts;
    // first graph's nodes for breadth first search:
    std::queue<Shape*> graphNodes;
    /*
     * for lcp
     */
...
*/
    ...
    // recursive call:
    static void batchIsland(Island* isl,Shape* coll);
    // recursive call:
    static void batchIsland_without_sleeping(Island* isl,Shape* coll);
    ...
    static void batchIslands(std::vector<Collision*>& colls,
        std::stack<Island*>&     islands);
    static void batchIslands_without_sleeping(
    ...
    void solve_stack_without_sleeping(Real         doit,
                                      unsigned int iterations_number);
    void solve_positions_without_sleeping(Real         dt,
                                          unsigned int iterations_number);
    void doit(Real doit);
    void solve(Real* J, Real* B, Real* nu, Real* lambda, Real* bounds, Real* a,
        int* idx, unsigned int s, unsigned int n,
        unsigned int iterations_number);
    /*
     * end: for lcp
     */

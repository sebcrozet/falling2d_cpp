...
pObject::pObject(Point2D *pts,int n,bool iscircle, World &w, bool cir,Point2D center)  : pts(pts), support(0), diskcenter(center)
{
	isdisk = cir;//iscircle;
	...
	srand(time(0));
	if(!cir && p->isFixed() || cir && d->isFixed())
	{
		r = 180;
		g = 180;
		b = 180;
...
void pObject::toogleFixed()
{
    ...
    srand(time(0));
    if(!isdisk && p->isFixed() || isdisk && d->isFixed())
    {
	r = 180;
	g = 180;
	b = 180;
